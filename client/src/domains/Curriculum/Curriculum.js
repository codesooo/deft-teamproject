...
import { Table, Modal, Input, Divider, Button } from 'antd';
...
const Curriculum = () => {
  ...
  const getData = async () => {
    await client.get('/api/course/list').then((res) => {
      setloading(false);
      setstate(
        res.data.map((row) => ({
          Title: row.title,
  ...
  const submitHandler = (e) => {
    ...
    client.post('/api/course/write', body).then((res) => console.log(res));
    alert('커리큘럼 등록 완료');
